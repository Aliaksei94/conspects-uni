...
\begin{Example}
 Function $\beta(x,s)=max(r,r^2)e^{-s}$ belongs to class $KL$.
\end{Example}

Properties of comparison functions:
...
Now we consider comparison functions and stability definitions.
...
Now consider comparison functions and Lyapunov functions
...
$\alpha_3 \circ \alpha_2^{-1} \in K$. By comparison lemma, $V(t,x(t))\le W(t)$, where $W$
solves $\dot W=-\alpha_3(\alpha_2^{-1}(W))$ with $W(t_0)=V(t_0,x(t_0))$. By auxiliary lemma
...
\begin{Theorem}
 ...
 continuously differentiable and $\frac{\partial f}{\partial x}$ bounded in $R^n$,
 uniformly in t ($||\frac{\partial f}{\partial x}(t,x)||\le L$ for all 
 $x\in R^n,\ t\ge 0,\ L>0$.

 If $x^*=0$ is globally exponentially stable, then exists differentiable 
...
\begin{proof}
Let $\Phi(\tau;t,x)$ -- solution to $\dot x(t)=f(t, x(t))$ which is static
at $(t,x)$.
